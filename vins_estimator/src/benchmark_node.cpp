...
void process_pose_graph()
{
    while(true)
    {
        ...
        if(index != -1)
        {
            ...
            CameraPoseVisualization* posegraph_visualization = keyframe_database.getPosegraphVisualization();
            m_update_visualization.unlock();
            pubOdometry(estimator, cur_header, relocalize_t, relocalize_r, img_id);
            pubPoseGraph(posegraph_visualization, cur_header);
...
void track_features()
{
  while(true)
  {
    ...
    {
      ROS_DEBUG("Feature tracker waiting for input images.");
      f_buf.unlock();
      std::chrono::milliseconds dura(10);
      std::this_thread::sleep_for(dura);
      continue;
...
int main(int argc, char **argv)
{
  ...
  std::thread feature_tracking{track_features};
  ...
  n.param("rosbag_filename", rosbag_filename, rosbag_filename);
  ...
  double last_msg_time = -1;
  double this_msg_time = -1;
  ...
  if(LOOP_CLOSURE)
  {
    ROS_INFO("Trying to join loop_detection");
    loop_detection.join();
    ROS_INFO("Trying to join pose_graph");
    pose_graph.join();
  }
